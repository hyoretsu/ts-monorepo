...
- **Scrollbars follow the theme.** Never leave a native browser scrollbar visible. Scrollable product UI must use the themed scrollbar utility; hide a scrollbar only when the scroll interaction remains obvious.
...
- **Sortable items use drag handles.** Reorderable groups, rows, or cards must use drag-and-drop through a visible drag-handle icon (never a numeric order field). Dragging an expanded group collapses it; dropping restores its previous expanded state.
